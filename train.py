...
class BertDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        data = torch.full([len(batch), self.max_token], self.pad_idx, device=label.device, dtype=batch[0]['data'].dtype) #建立一个二维tensor，宽度为一个batch，长度为最大词元数，初始化所有元素为self.pad_idx
...
parser.add_argument('--data', type=str, default='nyt', choices=['WebOfScience', 'nyt', 'rcv1','amazon'], help='Dataset.')
parser.add_argument('--batch', type=int, default=5, help='Batch size.')
parser.add_argument('--early-stop', type=int, default=6, help='E poch before early stop.')
parser.add_argument('--device', type=str, default='cuda')
parser.add_argument('--name', type=str, default='checkpoint01',  help='A name for different runs.')
parser.add_argument('--update', type=int, default=1, help='Gradient accumulate steps')
parser.add_argument('--warmup', default=2000, type=int, choices=[2000, 4000], help='Warmup steps.')
...
parser.add_argument('--lamb', default=0.03, type=float, help='Weight of contrastive learning.')
parser.add_argument('--thre', default=0.005, type=float, help='Threshold for keeping tokens. Denote as gamma in the paper.')
...
if __name__ == '__main__':
    ...
    label_dict = torch.load(os.path.join(data_path, 'bert_value_dict.pt'))  #导入
    ...
    model = ContrastModel.from_pretrained('bert-base-uncased', num_labels=num_class,
                                          ...
                                          NSL=args.nsl,gama_neg=args.gama_neg,gama_pos=args.gama_pos)
    if args.wandb:
        wandb.watch(model)
    ...
    log_file = open(os.path.join('checkpoints', args.name, 'log.txt'), 'w')  # 打开log文件
