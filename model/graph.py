...
class SelfAttention(nn.Module):
    ...
    def forward(self, hidden_states,attention_mask=None, output_attentions=False, extra_attn=None):
        residual = hidden_states
        hidden_states, attn_weights, _ = self.self(hidden_states=hidden_states, attention_mask=attention_mask,
                                                   output_attentions=output_attentions,extra_attn=extra_attn,)
...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output = (attn_output.view(bsz, self.num_heads, tgt_len,
                                        self.head_dim).transpose(1, 2).reshape(bsz, tgt_len, embed_dim))
...
class GraphLayer(nn.Module):
    def __init__(self, config, last=False):
        ...
        if GRAPH == 'GRAPHORMER':
            ...
            self.hir_attn = GATConv(config.hidden_size, config.hidden_size)
        elif GRAPH == 'GATv2':
            self.hir_attn = GATv2Conv(config.hidden_size, config.hidden_size)
        self.last = last
        if last:
    ...
    def forward(self, label_emb, extra_attn, self_attn_mask, inputs_embeds, cross_attn_mask):
        ...
        if self.last:
            ...
            label_emb = self.cross_attn(inputs_embeds, label_emb,attention_mask=cross_attn_mask.unsqueeze(1),
                                        output_attentions=True,only_attn=True)
            return label_emb
        label_emb = self.output_layer_norm(self.dropout(self.output_layer(label_emb)) + label_emb)
        if self.last:
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1):
        ...
        self.config = config
        self.tau = tau #温度超参数
        self.label_dict = torch.load(os.path.join(data_path, 'bert_value_dict.pt'))  #导入标签字典，key，value为标签值、标签序号
        self.tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased')  #导入词元化模型

        self.label_dict = {i: self.tokenizer.decode(v) for i, v in self.label_dict.items()}#将标签解码为文本
        self.label_name = []
        for i in range(len(self.label_dict)):
            self.label_name.append(self.label_dict[i])        #得到第i个样本的所有标签的名称
        self.label_name = self.tokenizer(self.label_name, padding='longest')['input_ids']   #得到切分后的标签名称对应的id
        self.label_name = nn.Parameter(torch.tensor(self.label_name, dtype=torch.long), requires_grad=False) #将标签名称在BERT词汇表中的id形成参数
        self.hir_layers = nn.ModuleList([GraphLayer(config, last=i == layer - 1) for i in range(layer)])

        self.label_num = len(self.label_name)  #样本数量

        self.graph = graph                     #图编码器
        ...
        if graph:
            ...
            path_dict = {}
            num_class = 0
            for s in label_hier:          #s为父亲，v为孩子，path_dict{孩子：父亲}
            ...
            if GRAPH == 'GRAPHORMER':
                num_class += 1
                for i in range(num_class):   #如果是根节点，则其父亲指向它自己
                    if i not in path_dict:
                        path_dict[i] = i
                self.inverse_label_list = {} #inverse_label_list为字典,里面键值对为{i:i的所有祖先-1}
                ...
                for i in range(num_class):
                    self.inverse_label_list.update({i: get_root(path_dict, i) + [-1]})  #这里为什么要+[-1]呢,+[-1]是在最后面加上-1这个元素
                ...
                self.edge_mat = torch.zeros(len(self.inverse_label_list), len(self.inverse_label_list), 15, dtype=torch.long)
                ...
                self.id_embedding = nn.Embedding(len(self.inverse_label_list) + 1, config.hidden_size,len(self.inverse_label_list))
            ...
            self.edge_list += [[v, i] for v, i in path_dict.items()]  #删除掉这行代码后，边只有从父亲到孩子的路径，没有孩子到父亲的路径
    ...
    def forward(self, inputs_embeds, attention_mask, labels, embeddings):
        label_mask = self.label_name != self.tokenizer.pad_token_id   #不为pad_token_id的标签名称向量都是label_mask
        # full name
        label_emb = embeddings(self.label_name)  #获得标签名称的嵌入
